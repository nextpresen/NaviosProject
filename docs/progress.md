...
- **セッション永続化（AsyncStorage）: ✅ 完了**
- **useAuth エラーハンドリング修正: ✅ 完了**
- **マイページ Supabase 実データ化: ✅ 完了**（プロフィール・統計・自分の投稿）
- **ゲスト閲覧対応: ✅ 完了**（未ログインでもタブ画面閲覧可、投稿作成・マイページは認証ガード）
- **Nearby PostCard 高さ固定: ✅ 完了**（カード可変サイズによる下コンテンツ被り解消）
- **検索画面セクション順変更: ✅ 完了**（トレンド→カテゴリ→過去の盛り上がり）
...
|---|---|---|
| 画面ファイル | **300行** | `app/` |
| コンポーネント | **150行** | `components/` |
| カスタムフック | **100行** | `hooks/` |

上限を超えたら即座に分割する。「あとでリファクタ」は禁止。

### コンポーネント切り出しの判断基準

以下のどれか1つでも当てはまれば `components/` に切り出す:

1. **2箇所以上で使う** → 共通コンポーネント化
2. **単体で意味をなすUI** → カード・リストアイテム・モーダル・バッジ等
3. **JSXが50行を超えるまとまり** → 画面ファイル内でも即分割

### ロジック分離の原則

- API呼び出し・副作用・状態管理 → **`hooks/` に切り出す**（画面ファイルに直書き禁止）
- マジックナンバー・設定値 → **`constants/` に定義**
- 共有する型 → **`types/` に定義**

### 悪い例 / 良い例

```tsx
// ❌ 悪い例: 画面ファイルに全部書く（400行超え）
...
// ✅ 良い例: ロジックをフックに、UIをコンポーネントに分離
export default function NearbyScreen() {
  const { posts } = usePosts(coords);   // hooks/usePosts.ts
  return <PostList posts={posts} />;    // components/post/PostList.tsx
}
```
